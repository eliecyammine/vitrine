...
const floatingShapes = [
  // Pen tool
  {
    x: "5%", y: "18%", size: 36, delay: 0, dur: 6,
    svg: (
      <svg viewBox="0 0 36 36" fill="none" className="w-full h-full">
        <path d="M26 4L32 10L14 28H8V22L26 4Z" stroke="var(--accent-secondary)" strokeWidth="1.5" strokeLinejoin="round" opacity="0.35" />
        <path d="M22 8L28 14" stroke="var(--accent-secondary)" strokeWidth="1" opacity="0.25" />
      </svg>
    ),
  },
  // Code bracket
  {
    x: "90%", y: "25%", size: 40, delay: 0.8, dur: 7,
    svg: (
      <svg viewBox="0 0 40 40" fill="none" className="w-full h-full">
        <path d="M14 10L6 20L14 30" stroke="var(--accent-color)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.3" />
        <path d="M26 10L34 20L26 30" stroke="var(--accent-color)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.3" />
      </svg>
    ),
  },
  // Lightbulb
  {
    x: "92%", y: "60%", size: 32, delay: 1.5, dur: 5,
    svg: (
      <svg viewBox="0 0 32 32" fill="none" className="w-full h-full">
        <circle cx="16" cy="13" r="7" stroke="var(--accent-color)" strokeWidth="1.5" opacity="0.3" />
        <path d="M13 20h6M14 23h4" stroke="var(--accent-color)" strokeWidth="1.5" strokeLinecap="round" opacity="0.3" />
      </svg>
    ),
  },
  // Star
  {
    x: "8%", y: "70%", size: 20, delay: 2, dur: 8,
    svg: (
      <svg viewBox="0 0 20 20" fill="none" className="w-full h-full">
        <path d="M10 2l2.5 5 5.5.8-4 3.9 1 5.5L10 14.5 4.5 17.2l1-5.5-4-3.9 5.5-.8L10 2z" stroke="var(--accent-secondary)" strokeWidth="1" opacity="0.25" />
      </svg>
    ),
  },
];
    ...
    <div className="relative overflow-hidden py-24 pt-32">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
        ...
        <motion.div
          ...
          className="absolute h-[500px] w-[500px] rounded-full bg-accent/[0.03] blur-[100px]"
        />
      </div>
        ...
        {floatingShapes.map((shape, i) => (
          <motion.div
            ...
            transition={{
              ...
              rotate: { duration: shape.dur * 1.4, repeat: Infinity, ease: "easeInOut" as const, delay: shape.delay },
            }}
        ...
        >
          ...
          >
            {[
              ...
              { value: "3", label: "Disciplines", sub: "Product · Design · Code" },
              ...
              >
                ...
                >
                  <span className="bg-gradient-to-b from-accent to-accent-secondary bg-clip-text text-transparent">
                    {stat.value}
                  </span>
                </motion.p>
                <p className="mt-2 text-sm font-medium text-text">{stat.label}</p>
                <p className="mt-0.5 text-xs text-muted">{stat.sub}</p>
                <div className="absolute bottom-0 left-0 h-px w-full bg-gradient-to-r from-transparent via-accent/0 to-transparent transition-all duration-500 group-hover:via-accent/40" />
              </motion.div>
            ))}
          </motion.div>
            ...
            <div className="relative overflow-hidden rounded-2xl border border-border/20 bg-card/30 py-4">
              ...
              <div className="animate-marquee flex w-max gap-6">
                {[...skills, ...skills].map((skill, i) => (
                  <span
                    key={`${skill}-${i}`}
                    className="flex-shrink-0 text-sm font-medium text-muted/70 transition-colors hover:text-accent"
                  >
                    {skill}
                  </span>
                ))}
